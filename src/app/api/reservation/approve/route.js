...
import { sendCustomerApprovalEmail } from "@/lib/emailService";
...
export async function GET(req) {
  try {
        ...
        `<html>
          <body style="font-family: sans-serif; text-align: center; background-color: #0d0c22; color: #fff; padding-top: 80px;">
            <div style="max-width: 600px; margin: 0 auto; background: #151446; padding: 40px; border-radius: 10px; border: 1px solid #dbbd82; box-shadow: 0 4px 15px rgba(0,0,0,0.5);">
              <h1 style="color: #dbbd82; font-size: 2.5rem; margin-bottom: 20px;">Bereits Bestätigt</h1>
              <p style="font-size: 1.2rem; line-height: 1.6; color: rgba(255,255,255,0.8);">
                Die Reservierung für <strong>${reservation.name}</strong> am <strong>${new Date(reservation.date).toLocaleDateString("de-DE")}</strong> wurde bereits zuvor bestätigt.
              </p>
            </div>
          </body>
        </html>`,
    ...
    await sendCustomerApprovalEmail(reservation);

    return new NextResponse(
      `<html>
        <body style="font-family: sans-serif; text-align: center; background-color: #0d0c22; color: #fff; padding-top: 80px;">
          <div style="max-width: 600px; margin: 0 auto; background: #151446; padding: 40px; border-radius: 10px; border: 1px solid #dbbd82; box-shadow: 0 4px 15px rgba(0,0,0,0.5);">
            <h1 style="color: #dbbd82; font-size: 2.5rem; margin-bottom: 20px;">Erfolgreich Bestätigt!</h1>
            <p style="font-size: 1.2rem; line-height: 1.6; color: rgba(255,255,255,0.8);">
              Die Reservierung für <strong>${reservation.name}</strong> am <strong>${new Date(reservation.date).toLocaleDateString("de-DE")}</strong> um <strong>${reservation.time}</strong> wurde bestätigt.
            </p>
            <p style="font-size: 1.1rem; color: #4CAF50; font-weight: bold; margin-top: 20px;">
              Eine Bestätigungs-E-Mail wurde an den Kunden gesendet.
            </p>
            <div style="margin-top: 40px; font-size: 0.9rem; color: rgba(255,255,255,0.4);">
              Leonn Restaurant | Deftere yazmayı unutmayın.
            </div>
          </div>
        </body>
      </html>`,
